      ...
      <div>
        <div class="jumbotron">
          ...
          <table class="table table-bordered">
            ...
            <?php $no=0 ;foreach ($jurusan as $rows): $no++?>
               <tr>
                <td><?= $no ?></td>
                <td><?= $rows->kode_jurusan ?></td>
                 <td><?= $rows->nama_jurusan ?></td>
                 <td align="center">
                   <a id="<?= $rows->kode_jurusan ?>" class="btn btn-danger btn-sm btn-edit">Edit</a>
                   &nbsp;
                   <a onclick="return confirm('Apakah anda ingin menghapus data ini?')" href="<?= base_url('app/jurusanAction/delete/'.$rows->kode_jurusan); ?>" class="btn btn-primary btn-sm">Delete</a>
                 </td>
               </tr>
            <?php endforeach ?>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="form-group">
              ...
              <input type="text" name="kode_jurusan" class="form-control" placeholder="Kode Jurusan" required>
            </div>
          </div>
          <div class="col-md-12">
            <div class="form-group">
    ...
    <script type="text/javascript">
      $(document).ready(function() {
        $('#btn-add').on('click', function(event) {
          ...
          $('input[name="kode_jurusan"]').prop('readonly',false);
        });
        ...
        $('.btn-edit').on('click', function(event) {
          ...
          var id = this.id;
          $.ajax({
            url: '<?= base_url('app/jurusanAction/edit'); ?>',
            type: 'POST',
            dataType: 'JSON',
            data: {id: id},
            success: function(x){
              ...
              $('input[name="kode_jurusan"]').prop('readonly',true);
        ...
        $('#form-petugas').on('submit', function(event) {
          ...
          {
            $.ajax({
              url: '<?= base_url('app/jurusanAction/insert'); ?>',
              type: 'POST',
              dataType: 'JSON',
              data: $(this).serialize(),
              success: function(x){
          ...
          {
            $.ajax({
              url: '<?= base_url('app/jurusanAction/update'); ?>',
              type: 'POST',
              dataType: 'JSON',
              data: $(this).serialize(),
              success: function(x){
